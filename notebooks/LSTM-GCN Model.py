...
def train_model(project_root=os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))):
    ...
    optimizer = optim_lookahead.Lookahead(base_optimizer, k=5,
                                          alpha=0.5)  # k: synchronization steps, alpha: slow weights update
    ...
    for model_index in range(num_models):
        ...
        for epoch in range(num_epochs):
            ...
            print(
                f"Model {model_index + 1}, Epoch [{epoch + 1}/{num_epochs}], Loss: {avg_loss:.4f} (EMA: {running_loss:.4f})")
    ...
    for model_index in range(num_models):
        ...
        for symbol in symbols:
            ...
            comparison_df = pd.DataFrame({'Actual Price': true_prices, 'Predicted Price': predicted_prices},
                                         index=test_index)
            ...
            plt.plot(comparison_df.index, comparison_df['Predicted Price'], label=f'Predicted {symbol} Price',
